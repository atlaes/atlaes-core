...
5. **Item 19 (July design):** EUR-account sub-flow is `Eligibility/VBL-7.png` and `VBL-12.png` (July, this worktree) — SummitFX is named in body copy and shown as a logo/wordmark with a "Continue with SummitFX" button. Read both screenshots and match copy and structure. For the logo: no vector asset exists in the repo — render the name as styled text matching the wordmark for now and leave `// TODO(asset): replace text wordmark with official SummitFX logo asset`. Do not crop the logo out of the PNG.
...
2. Two-phase component like `Identity.tsx` (upload → OCR-less confirm is fine: document extraction for HI is NOT required this round — pre-fill nothing, let the user type; keep the upload as a stored document attached to the claim). Do not build a new OCR pipeline.
3. Backend: investigate how identity/address/bank fields are persisted on the claim record and mirror that pattern for health insurance fields (type, provider name, provider address, insuredSinceMonth, insuredSinceYear, placeOfBirth, countryOfBirth, insuranceNumber, documentId). Generate the Drizzle migration; run backend tests for the claims service you touch (`pnpm vitest run <relevant test file>` from packages/functions) — pre-existing unrelated failures are not yours.
